...
subroutine buildSparseMatrixSGNcrs(q,qold,aux,soln,rhs,rowPtr,cols,vals,   &
    ...
    real(kind=8) :: sa, huc, hvc, rowSum
    integer :: ja, bc, c, indexSt, ind
        ...
        do j=1,ny 
            ...
            do i=1,nx
              ...
              ! for u, and colPtr+12 to colPtr+23 for v
                 ...
                 do ind = indexSt,indexSt+11
                   rowSum = rowSum + vals(ind)
                 end do
                 if (rowSum .lt. 0.d0) then
                      if (firstTime) then
                      write(*,888) mptr, levelBouss
 888  format("Working on grid ",i5," level ",i5)
                      firstTime = .false.
                      endif
                    ...
                    write(*,*)" i,j = ",i,j, "     negative u row sum , nx,ny ",nx,ny,"     ghost"
                    else
                    write(*,*)" i,j = ",i,j, "     negative u row sum , nx,ny ",nx,ny," not ghost"
                    endif
                 endif
                 rowSum = 0.d0
                 indexSt = rowPtr(2*k_ij-1)
                 ...
                 if (rowSum .lt. 0.d0) then
                    if (IS_GHOST(i,j)) then 
                    write(*,*)" i,j = ",i,j, "     negative v row sum , nx,ny ",nx,ny,"     ghost"
                    else
                    write(*,*)" i,j = ",i,j, "     negative v row sum , nx,ny ",nx,ny," not ghost"
                    endif
                 endif
              if (IS_GHOST(i,j)) then 
                 indexSt = rowPtr(2*(k_ij-1))
